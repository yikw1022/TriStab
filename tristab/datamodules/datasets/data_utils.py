# https://github.com/BytedProtein/ByProt/blob/dd279dc85f76ee2c28c819b71bf3911b90159f0a/src/byprot/datamodules/datasets/data_utils.py
import heapq
import itertools
import math
import os
...
class Alphabet(object):
    def __init__(self, name='esm', featurizer='cath', alphabet_cfg={}, featurizer_cfg={'single_mut': False,'mut_seq':False}):
        self.name = name
        self._alphabet = None
    ...
    def get_featurizer(self, name='cath', **kwds):
        if name == 'cath':
            ...
            from .ddg import Featurizer
            return Featurizer(alphabet=self, 
                              to_pifold_format=kwds.get('to_pifold_format', False),
                              coord_nan_to_zero=kwds.get('coord_nan_to_zero', True))
        elif name == 'megascale':
            from .megascale import Featurizer
            return Featurizer(alphabet=self, 
                              to_pifold_format=kwds.get('to_pifold_format', False),
                              coord_nan_to_zero=kwds.get('coord_nan_to_zero', True),
