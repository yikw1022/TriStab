# https://github.com/BytedProtein/ByProt/blob/dd279dc85f76ee2c28c819b71bf3911b90159f0a/src/byprot/utils/__init__.py
...
def common_pipeline(config, training=False):
    ...
    if 'model' in config.task:
        pl_module: LightningModule = instantiate_from_config(cfg=config.task, group='task')
    else:
    ...
    if "callbacks" in config:
        ...
        if config.trainer.get('enable_progress_bar', False):
            from tristab.utils.callbacks import BetterRichProgressBar
...
def get_git_revision_hash() -> str:
    try:
        ...
        return "development"  # 鎴栬€呰繑鍥炰换浣曚綘鎯宠鐨勯粯璁ゅ€?
